...
class WebTests(unittest.TestCase):
    ...
    def test_pick_session_explains_research_value_fit(self):
        ...
        self.assertIn('<span class="fit-chip ready" title="${escapeHtml(readySummaryTitle)}">Ready ${readyCount}</span>', INDEX_HTML)
        self.assertIn('<span class="fit-chip improve" title="Not ready yet: needs more turns or a context compaction">Keep chatting ${counts.improve || 0}</span>', INDEX_HTML)
        self.assertIn("const readyCount = (counts.best || 0) + (counts.good || 0) + (counts.long || 0)", INDEX_HTML)
        self.assertIn("const readySummaryTitle = `Best: ${counts.best || 0}\\nGood: ${counts.good || 0}\\nLong: ${counts.long || 0}`", INDEX_HTML)
        ...
        self.assertIn("$('sessionCount').title = sessionSummaryTitle", INDEX_HTML)
