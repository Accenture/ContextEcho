...
class WebTests(unittest.TestCase):
    ...
    def test_pick_session_explains_research_value_fit(self):
        ...
        self.assertIn('class="session-search-wrap"', INDEX_HTML)
        self.assertIn('placeholder="Search sessions, agent, model, project"', INDEX_HTML)
        self.assertIn(".session-search-wrap { position:relative; flex:0 1 380px; max-width:380px; min-width:240px; }", INDEX_HTML)
        self.assertIn('id="columnsBtn"', INDEX_HTML)
        self.assertIn('id="columnsMenu"', INDEX_HTML)
        self.assertIn("let sessionColumnVisibility = {date:true, turns:true, cmp:true, fit:true}", INDEX_HTML)
        self.assertIn("function applySessionColumnVisibility()", INDEX_HTML)
        self.assertIn("hide-col-date", INDEX_HTML)
        self.assertIn("&vellip;</button>", INDEX_HTML)
        self.assertIn("&lsaquo; Prev", INDEX_HTML)
