...
class WebTests(unittest.TestCase):
    ...
    def test_not_ready_sessions_offer_resume_project_actions(self):
        ...
        self.assertIn("Copy exact resume command", INDEX_HTML)
        self.assertIn("Open project folder", INDEX_HTML)
        self.assertIn("post('/api/open_path', {path:resumeInfo.dir})", INDEX_HTML)
        self.assertIn("Resume this session in your agent", INDEX_HTML)
        self.assertIn("Run this exact resume command to continue this specific session.", INDEX_HTML)
        self.assertIn("Session ID: <code>${escapeHtml(resumeGuidance.resumeId)}</code>", INDEX_HTML)
        self.assertIn("Use this project folder, start the agent, then type <code>/resume</code>", INDEX_HTML)
        self.assertIn("Project folder: <code>${escapeHtml(resumeGuidance.dir)}</code>", INDEX_HTML)
        self.assertIn("Copy command", INDEX_HTML)
        self.assertIn("Resume command copied. See the highlighted instruction above the session list.", INDEX_HTML)
        self.assertIn("Project folder opened. The highlighted instruction shows the /resume step.", INDEX_HTML)
        self.assertIn("resume cmd", INDEX_HTML)
        self.assertIn("Use the resume cmd pill or row arrow to copy the exact agent resume command", INDEX_HTML)
