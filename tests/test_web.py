...
class WebTests(unittest.TestCase):
    ...
    def test_donated_rows_show_copyable_support_submission_id(self):
        ...
        self.assertIn("data-session-action=\"update-info\"", INDEX_HTML)
        ...
        self.assertIn(".resume-guidance.donation-guidance { border-color:#168147;", INDEX_HTML)
        self.assertIn("el.classList.add('donation-guidance');", INDEX_HTML)
        self.assertIn("el.classList.remove('donation-guidance');", INDEX_HTML)
        ...
        self.assertNotIn(".session-row[data-tooltip]", INDEX_HTML)
        self.assertNotIn("selectedUpdateInfo", INDEX_HTML)
        ...
        self.assertNotIn("data-session-action=\"copy-resume\"", INDEX_HTML)
        self.assertNotIn("data-session-action=\"open-resume\"", INDEX_HTML)
        ...
        self.assertNotIn("prefillContributorFields(selected, false)", INDEX_HTML)
        ...
        self.assertIn("Report issue", INDEX_HTML)
    ...
    def test_not_ready_sessions_offer_resume_project_actions(self):
        ...
        self.assertIn("function renderResumeGuidance()", INDEX_HTML)
        self.assertIn("function clearResumeGuidance({rerender=false} = {})", INDEX_HTML)
        self.assertIn("function copyResumeSteps(session, resumeInfo)", INDEX_HTML)
        self.assertIn("function openResumeFolder(session, resumeInfo)", INDEX_HTML)
        self.assertIn("const dir = String(s?.resume_dir || '').trim()", INDEX_HTML)
        self.assertIn("const exactCommand = String(s?.resume_command || '').trim()", INDEX_HTML)
        self.assertIn("const resumeId = String(s?.resume_session_id || '').trim()", INDEX_HTML)
        ...
        self.assertIn(".session-row.resume-focus-row", INDEX_HTML)
        self.assertIn("if(resumeGuidance?.key === resumeKey) row.classList.add('resume-focus-row')", INDEX_HTML)
        self.assertNotIn('data-session-action="resume"', INDEX_HTML)
        self.assertNotIn('data-session-action="copy-resume"', INDEX_HTML)
        self.assertNotIn('data-session-action="open-resume"', INDEX_HTML)
        self.assertNotIn("oncontextmenu", INDEX_HTML)
        self.assertIn("Copy add-turns command", INDEX_HTML)
        self.assertIn("Open folder", INDEX_HTML)
        self.assertIn("post('/api/open_path', {path:resumeInfo.dir})", INDEX_HTML)
        self.assertIn("Not ready yet: add turns first", INDEX_HTML)
        self.assertIn("This session needs 50+ turns before donation", INDEX_HTML)
        self.assertIn("Copy the exact command below, keep chatting in the agent, then come back and donate.", INDEX_HTML)
        self.assertIn("Session ID: <code>${escapeHtml(resumeGuidance.resumeId)}</code>", INDEX_HTML)
        self.assertIn("Use this project folder, start the agent, type <code>/resume</code>, keep chatting, then come back and donate.", INDEX_HTML)
        ...
        self.assertIn("status('discoverStatus', '')", INDEX_HTML)
        self.assertIn("document.querySelectorAll('.resume-focus-row').forEach", INDEX_HTML)
        self.assertIn("clearResumeGuidance({rerender:true})", INDEX_HTML)
        self.assertIn("if(row && row.dataset.sessionKey === resumeGuidance.key) return;", INDEX_HTML)
        self.assertIn("page--; clearResumeGuidance(); clearSelectedSession(); renderSessions();", INDEX_HTML)
        self.assertIn("page++; clearResumeGuidance(); clearSelectedSession(); renderSessions();", INDEX_HTML)
        self.assertNotIn("resume cmd", INDEX_HTML)
        self.assertNotIn('data-session-action="continue"', INDEX_HTML)
        self.assertNotIn("Copy continue steps", INDEX_HTML)
    ...
    def test_maintainer_metadata_updates_can_be_approved(self):
        ...
        self.assertIn('id="globalLookup"', html)
        self.assertIn("Find donation", html)
        self.assertIn("function runGlobalLookup", html)
        self.assertIn("function globalLookupSections", html)
        self.assertIn("function findGlobalLookupMatch", html)
        self.assertIn("applyGlobalLookupMatch", html)
        self.assertIn("$('globalLookupBtn').onclick", html)
        ...
        self.assertIn("/api/admin/redaction-search", html)
        self.assertIn("id=\"openRedactionRepair\"", html)
        self.assertIn("Open repair tools", html)
        self.assertIn("id=\"redactionRepairCard\" class=\"card repair-card\" hidden", html)
        self.assertIn("id=\"closeRedactionRepair\"", html)
        self.assertIn("Repair redaction on a submitted item", html)
        self.assertIn("Single submission ID", html)
        self.assertIn("Batch redaction uses the selected rows in the table", html)
        self.assertIn("id=\"selectedRedactionSummary\"", html)
        self.assertIn("Selected for batch", html)
        self.assertIn("Redact single", html)
        self.assertIn("Check single ID", html)
        self.assertIn("Redact single and check again", html)
        self.assertIn("id=\"redactionCheckBatch\"", html)
        self.assertIn("Use Redact single for the checked submission", html)
        ...
        self.assertIn("id=\"applySelectedRedactions\"", html)
        self.assertIn("id=\"checkRedactionTerms\"", html)
        self.assertIn("function applyRedactionUpdate", html)
        self.assertIn("function applySelectedRedactionUpdates", html)
        self.assertIn("function setRedactionRepairOpen", html)
        self.assertIn("openRedactionRepair('redactionTerms')", html)
        self.assertIn("function postRedactionSearch", html)
        self.assertIn("function redactionSearchSummary", html)
        self.assertIn("renderBatchRedactionProgress", html)
        self.assertIn("Before:", html)
        self.assertIn("Checking redaction", html)
        self.assertIn("selectedRedactionSubmissions:new Set()", html)
        self.assertIn("data-redaction-select", html)
        ...
        self.assertIn("{label:'50-99', count:0}", html)
        self.assertIn("{label:'500-999', count:0}", html)
        self.assertIn("{label:'1000+', count:0}", html)
        self.assertNotIn("{label:'0-49', count:0}", html)
        ...
        self.assertIn("pendingGroupByDonor:true", html)
        self.assertIn("pendingGroupSortKey:'donation_date'", html)
        self.assertIn("pendingGroupSortDir:'desc'", html)
        self.assertIn("expandedDonorGroups:new Set()", html)
        self.assertIn("function donorGroups", html)
        self.assertIn("function renderGroupedPendingTable", html)
        self.assertIn("function renderFlatPendingTable", html)
        self.assertIn("function setPendingGroupSort", html)
        self.assertIn("function groupSortButton", html)
        self.assertIn("Group by donor", html)
        self.assertIn("Show flat rows", html)
        self.assertIn("Donation date", html)
        self.assertIn("Donations", html)
        self.assertIn("data-group-sort", html)
        self.assertIn("latestSubmitted", html)
        self.assertIn("Expand all", html)
        self.assertIn("Collapse all", html)
        self.assertIn("Select donor sessions", html)
        self.assertIn("Grouped by exact donor identity: name, email, and institute.", html)
        self.assertIn("data-donor-toggle", html)
        self.assertIn("data-donor-select", html)
    ...
    def test_maintainer_pending_view_separates_promoted_from_needs_validation(self):
        ...
        self.assertIn("acceptedSubmissionIds:new Set()", html)
        self.assertIn("acceptedSubmissionPublicIdentities:{}", html)
        self.assertIn("function loadPublicAcceptedSubmissionMetadata", html)
        self.assertIn("accepted_submission_public_identities", html)
        self.assertIn("public_display_name:publicIdentity.public_display_name", html)
        self.assertIn("'public_anonymous' in publicIdentity", html)
        self.assertIn("fetch('../project_stats.json', {cache:'no-store'})", html)
        self.assertIn("function normalizePendingRows", html)
        self.assertIn("accepted release ledger", html)
    ...
    def test_submit_requires_contributor_identity_fields(self):
        ...
        self.assertIn('id="contributorName" placeholder="your name or handle" autocomplete="off" required', INDEX_HTML)
        self.assertIn('id="contributorEmail" type="email" list="emailSuggestions" placeholder="you@example.com" autocomplete="off" required', INDEX_HTML)
        self.assertIn('datalist id="emailSuggestions"', INDEX_HTML)
        self.assertIn('id="contributorInstitute" placeholder="University / company / independent" autocomplete="off" required', INDEX_HTML)
        self.assertIn("function contributorFieldsComplete()", INDEX_HTML)
        self.assertIn("function clearContributorFields()", INDEX_HTML)
        self.assertIn("clearContributorFields();", INDEX_HTML)
        self.assertIn("const lastContributorInfoKey = 'contextechoLastContributorInfoV1';", INDEX_HTML)
        self.assertIn("let lastContributorInfo = loadLastContributorInfo();", INDEX_HTML)
        self.assertIn("function applyContributorInfo(info, overwrite=false)", INDEX_HTML)
        self.assertIn("function contributorInfoHasFields(info)", INDEX_HTML)
        self.assertIn("function rememberContributorFields()", INDEX_HTML)
        self.assertIn("function restoreLastContributorFields()", INDEX_HTML)
        self.assertIn("lastContributorInfo = currentContributorInfo();", INDEX_HTML)
        self.assertIn("localStorage.setItem(lastContributorInfoKey, JSON.stringify(lastContributorInfo));", INDEX_HTML)
        self.assertIn("const selectedContributor = selected ? localContributorRecord(selected) : null;", INDEX_HTML)
        self.assertIn("applyContributorInfo(selectedContributor, false);", INDEX_HTML)
        self.assertIn("applyContributorInfo(lastContributorInfo, false);", INDEX_HTML)
        self.assertEqual(INDEX_HTML.count("resetSessionArtifacts(); restoreLastContributorFields(); goStep(1);"), 2)
    ...
    def test_donated_sessions_do_not_offer_local_clear_action(self):
        ...
        self.assertIn("Showing saved donation info", INDEX_HTML)
        self.assertIn("Use the row buttons to update info or report an issue", INDEX_HTML)
        self.assertIn("update ready", INDEX_HTML)
        self.assertIn("new turns", INDEX_HTML)
        self.assertIn("contextechoDonatedRecordsV1", INDEX_HTML)
        self.assertIn("function sessionUpdateReady(currentTurns, previousTurns)", INDEX_HTML)
        self.assertIn("sessionUpdateReady(currentTurns, previousTurns)", INDEX_HTML)
        self.assertNotIn("session_update_ready(currentTurns, previousTurns)", INDEX_HTML)
    ...
    def test_pick_session_explains_research_value_fit(self):
        ...
        self.assertIn(".count-badge.active, .fit-chip.active", INDEX_HTML)
        self.assertIn('id="sessionCount" class="count-badge active" data-session-filter="all"', INDEX_HTML)
        self.assertIn("white-space:pre-line", INDEX_HTML)
        self.assertIn('data-session-filter="donated"', INDEX_HTML)
        self.assertIn('data-session-filter="ready"', INDEX_HTML)
        self.assertIn('data-session-filter="improve"', INDEX_HTML)
        ...
        self.assertIn("function sessionIsActionableReady(s)", INDEX_HTML)
        self.assertIn("function sessionNeedsMoreTurns(s)", INDEX_HTML)
        self.assertIn("const selectedActionable = !!(selected && sessionIsActionableReady(selected));", INDEX_HTML)
        self.assertIn("$('pickNext').disabled = !selectedActionable", INDEX_HTML)
        self.assertIn("function clearSelectedSession()", INDEX_HTML)
        self.assertIn("clearResumeGuidance(); clearSelectedSession(); renderSessions();", INDEX_HTML)
        self.assertIn("function actionableReadyCounts()", INDEX_HTML)
        self.assertIn("const readyCount = (readyCounts.best || 0) + (readyCounts.good || 0)", INDEX_HTML)
        self.assertIn("const improveCount = sessions.reduce((count, s) => count + (sessionNeedsMoreTurns(s) ? 1 : 0), 0)", INDEX_HTML)
        self.assertIn("const donatedTotal = sessions.reduce((count, s) => count + (sessionIsBlockedDonation(s) ? 1 : 0), 0)", INDEX_HTML)
        self.assertIn("const readySummaryTitle = `Best: ${readyCounts.best || 0}\\nExcellent: ${readyCounts.good || 0}`", INDEX_HTML)
        self.assertIn("function donatedModelCounts()", INDEX_HTML)
        self.assertIn("function donatedModelSummary(maxItems = 5)", INDEX_HTML)
        ...
        self.assertNotIn("function fitCounts()", INDEX_HTML)
        ...
        self.assertIn('class="sort-header" data-sort-key="compactions" data-tooltip="Context compactions detected in local logs." aria-label="Context compactions detected in local logs."', INDEX_HTML)
        self.assertIn('data-sort-key="compactions" aria-sort="${aria(\'compactions\')}" data-tooltip="Context compactions detected in local logs." aria-label="Context compactions detected in local logs.">Ctx cmp', INDEX_HTML)
        ...
        self.assertNotIn("if(value === 'long')", INDEX_HTML)
        ...
        self.assertIn("t>=100&&c>=2?'best':(t>=50?'good':'improve')", INDEX_HTML)
        ...
        self.assertIn('placeholder="Search sessions, models"', INDEX_HTML)
        self.assertIn(".session-search { flex:0 0 220px; max-width:220px; min-width:180px; }", INDEX_HTML)
        self.assertNotIn('id="sessionSortMode"', INDEX_HTML)
        self.assertNotIn('<option value="model">Group by model</option>', INDEX_HTML)
        self.assertNotIn("let sessionSortMode", INDEX_HTML)
        self.assertIn("let sessionSearchQuery = ''", INDEX_HTML)
        self.assertIn("let sessionStatusFilter = 'all'", INDEX_HTML)
        self.assertIn("function sessionStatusMatches(s)", INDEX_HTML)
        self.assertIn("if(sessionStatusFilter === 'ready') return sessionIsActionableReady(s)", INDEX_HTML)
        self.assertIn("if(sessionStatusFilter === 'improve') return sessionNeedsMoreTurns(s)", INDEX_HTML)
        self.assertIn(".filter(item => sessionStatusMatches(item.s))", INDEX_HTML)
        self.assertIn("function bindSessionFilterControls()", INDEX_HTML)
        self.assertIn("sessionStatusFilter = sessionStatusFilter === nextFilter && nextFilter !== 'all' ? 'all' : nextFilter", INDEX_HTML)
        self.assertIn("clearSelectedSession();\n      renderSessions();", INDEX_HTML)
        ...
        self.assertIn("if(!sessionSort.key) return items", INDEX_HTML)
    ...
    def test_donation_fit_thresholds(self):
        ...
        self.assertEqual(donation_fit(50, 0), "good")
        self.assertEqual(donation_fit(100, 0), "good")
        self.assertEqual(donation_fit(49, 1), "improve")
        self.assertTrue(donation_ready(50, 1))
        self.assertTrue(donation_ready(50, 0))
