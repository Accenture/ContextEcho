...
class WebTests(unittest.TestCase):
    ...
    def test_pick_session_explains_research_value_fit(self):
        ...
        self.assertIn('>Keep chatting ${counts.improve || 0}</button>', INDEX_HTML)
        self.assertIn("const readyCount = (counts.best || 0) + (counts.good || 0)", INDEX_HTML)
        self.assertIn("const donatedTotal = sessions.reduce((count, s) =>", INDEX_HTML)
        self.assertIn("const readySummaryTitle = `Best: ${counts.best || 0}\\nExcellent: ${counts.good || 0}`", INDEX_HTML)
