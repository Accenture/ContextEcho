"""Fig 1 motivation data: 25 probes × 12 positions × 2 arms × Opus 4.7.

Single target (claude-opus-4-7), 600 cells total.
~$15-20, ~1-1.5 hr wall.

Output: results/probes_at_crosscompaction/claude-opus-4-7/<position>/
...
def main() -> int:
    ...
    print(f"Positions:        {len(ALL_POSITIONS)} ({ALL_POSITIONS})")
    print(f"Probes:           {len(ALL_PROBES)}")
    print(f"Total cells:      {len(ALL_POSITIONS) * len(ALL_PROBES) * 2}")
    ...
    for pos_label in ALL_POSITIONS:
        ...
        print(f"  {pos_label} done: cum {n_done}/{n_total}, elapsed {elapsed:.0f}s")
