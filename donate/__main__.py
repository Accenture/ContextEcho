...
def quality_tag(session: dict) -> str:
    ...
    if turns >= 100 and compactions >= 2:
        return "best"
    if turns >= 50 and compactions >= 1:
        return "good"
    if turns >= 100:
        return "long"
    return "improve"
...
def print_session_table(sessions: list[dict], start: int = 0, limit: int = 15) -> None:
    ...
    print("  Fit: best = 100+ user turns and 2+ CCmp; good = 50+ turns and 1+ CCmp; long = 100+ turns; improve = keep chatting.")
    if end < len(sessions):
